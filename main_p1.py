...
import numpy as np
import meshio
from vedo import dolfin

#import os

with meshio.xdmf.TimeSeriesReader("data/deformed/cube_tetra_1_solution.xdmf") as reader:
...
beam_q1 = Mesh('./data/undeformed/cube_tetra_1.vtu', c='black').wireframe().lineWidth(0.1).x(50)

deformed_q1 = Mesh('data/deformed/tetra_deformed.vtu', c='black').lineWidth(0.1).lighting('').x(50) 
#dep = deformed_q1.points()[:, 1]

forces_points_initial = deformed_q1.points()[-9:-1] 
forces_points_initial_1 = deformed_q1.points()[-13:-10] 
...
a1 = Arrows(forces_points_initial, forces_points_final, c='blue')
a2 = Arrows(forces_points_initial_1, forces_points_final_1, c='blue')
...
deformed_q1.cmap(mycmap, dep).addScalarBar(nlabels=3)
...
plt = vedo.Plotter()
plt.add([ltx_gamma_D, ltx_omega, ltx_gamma_N,  beam_q1, a1, a2, deformed_q1, fixed_plan2, ])
plt.show().close()